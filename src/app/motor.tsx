...
export default function MotorScreen() {
  ...
  const [nlvRpm, setNlvRpm] = useState("");
  const [isShow, setIsShow] = useState(false);
  const [catalog, setCatalog] = useState<MotorDkRow[]>([]);
  const [snapshot, setSnapshot] = useState<MotorCalcSnapshot | null>(null);
  ...
  const runCalculation = useCallback(() => {
    ...
    const motorsTop2 = selectMotorDkStep4Top(catalog, P_ct, n_sb, 2);
